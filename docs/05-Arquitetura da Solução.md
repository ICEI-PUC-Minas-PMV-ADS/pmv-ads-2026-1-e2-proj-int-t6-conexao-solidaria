...
Ex: 'Beneficiário', 'Doador', 'Voluntário', 'Administrador'.
localizacao	VARCHAR	

Endereço ou coordenadas (Específico de Beneficiário).
necessidadeAtual	TEXT	

Resumo da situação (Específico de Beneficiário).
...
Identificador único.
id_beneficiario	FK (INT)	

Referência ao Usuário que solicitou.
descricao	TEXT	
...
Ex: 'Aberto', 'Atendido', 'Cancelado'.
dataCriacao	DATETIME	

Data e hora do registro.
localizacao	VARCHAR	

Local específico para este pedido.
...
Referência ao Usuário que realiza a doação.
id_pedido_ajuda	FK (INT)	

Referência ao pedido que esta doação atende.
itensDoados	TEXT	

Descrição do que está sendo doado.
status	VARCHAR	

Ex: 'Pendente', 'Entregue'.
dataDoacao	DATETIME	

Data do registro/efetivação.
...
Nome do grupo (pode ser nulo para chats 1-pra-1).
dataCriacao	DATETIME	

Quando o chat/grupo foi criado.
...
Referência ao ChatApoio onde a mensagem foi enviada.
id_remetente	FK (INT)	
